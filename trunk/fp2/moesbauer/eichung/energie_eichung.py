...
##        lg.AddEntry(self.fitfkt, 'Gau�fit', 'l')
...
for line in open('Eisen.IEC','r'):
    if dataline == 0:
        ...
        for word in line.split()[2:]:
            if word != "":
                cou.append(float(word))
n = len(cou)
cs = [i for i in range(n)]
sx = [1]*n
sy = [1/(sqrt(i+1)) for i in cou]
g1 = TGraphErrors(n, array('d',cs) ,array('d',cou),array('d',sx),array('d',sy))
g1.SetTitle(';Energie [keV];Kanal')
g1.GetHistogram().SetTitleOffset(1.3, 'Y')
g1.SetMarkerStyle(3)
g1.SetMarkerColor(2)
g1.SetMarkerSize(1.0)
f1 = TF1('f_prove', '(([0]/sqrt(2*pi*[1])) * exp(-0.5*(((x-[2])/[1])^2))) + [3]', 1150, 1800)
f1.SetMarkerColor(2)
f1.SetParameters(array('d', [500, 100, 1300, 5]))
g1.Fit(f1, 'MQRE', 'X')
ort, sort = f1.GetParameter(2), f1.GetParError(2)
chisq1 = f1.GetChisquare()
ndf1 = f1.GetNDF()
rchisq1 = chisq1/ndf1
c = TCanvas('Eichung', 'eichung')
c.SetGrid()
